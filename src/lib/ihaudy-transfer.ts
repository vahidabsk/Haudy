...
export async function importFieldNotesFromIHaudy(file: File, group: AssignmentGroup) {
  ...
  await initializePhotoStore(existingAudits);
  await importPayloadPhotos(payload.audits);
...
async function importPayloadPhotos(audits: Audit[]) {
  for (const audit of audits) {
    for (const rows of [audit.documentation, audit.installation, audit.deviceTests]) {
      for (const row of rows) {
        const stored: string[] = [];
        for (const photo of row.photos || []) {
          stored.push(photo.startsWith("data:") ? await storePhotoDataUrl(photo) : photo);
        }
        row.photos = stored;
      }
    }
