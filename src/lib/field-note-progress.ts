...
export function fieldNoteProgress(audit: Audit): FieldNoteProgress {
  ...
  if (program !== "mercantile") {
    total += 1;
    const signalAddressed = Boolean(audit.deviceSystemLocal || audit.editedFields?.signalProcessingReviewed);
  ...
  total += 1;
  const documentationAddressed = Boolean(audit.editedFields?.documentationReviewed);
  ...
  total += 1;
  const installationAddressed = Boolean(audit.editedFields?.installationReviewed);
  ...
  total += 1;
  const deviceAddressed = Boolean(audit.editedFields?.deviceTestingReviewed);
