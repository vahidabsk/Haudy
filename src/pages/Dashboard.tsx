              ...
              {group.audits.map((audit) => (
                <article key={audit.id} className="grid gap-3 rounded-md border bg-slate-50 p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Link className="min-h-11 rounded-md border bg-white px-4 py-2 text-center" to={`/audit/${audit.id}/export`}>Export</Link>
                    <button className="min-h-11 rounded-md border bg-white px-4 py-2" onClick={() => audits.deleteAudit(audit.id)}>Delete</button>
                  </div>
                </article>
              ))}
            </div>
